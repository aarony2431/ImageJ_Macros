...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
function convert_to_um(unit_from) {
	units = newArray("inch", "inches", "mm", "cm", "m", "um", "µm");
	convertor_to_um = newArray(25400, 25400, 1000, 10000, 1000000, 1, 1);
	for (i = 0; i < units.length; i++) {
		if (unit_from == units[i]) {
			return convertor_to_um[i];
		}
	}
	print("Cannot identify image unit length! Default is set to \"pixel\".");
	return 1;
}
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
fiber_bin_class_separator = 250;
fiber_bin_class_min_area = 250;
fiber_bin_class_max_area = 7000;
num_fiber_bins = Math.max(1, Math.floor((fiber_bin_class_max_area-fiber_bin_class_min_area)/fiber_bin_class_separator));
function AreaClassAttribution(Area,FiberAreaClassCount) {
	bin = Math.floor(Math.max(Area - fiber_bin_class_min_area, 0)/fiber_bin_class_separator);
	bin = Math.min(bin, num_fiber_bins-1);
	FiberAreaClassCount[bin] = FiberAreaClassCount[bin] + 1;
}
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
function NucleiDetectionOnDAPI(System,DAPIWindows) {
	...
	run("Select All"); // added 2021.11.08 Aaron Yu
...
function ArtefactDetectionOnLaminin(FiberImageWindows, bCrop, LimitDetectionFiberArea, bArtefacts) {
	...
	if (setscale) {
		run("Set Scale...", "distance="+dpm+" known=1 unit=um");
	}
	run("Duplicate...", "title=[Fiber Temp]");
	selectWindow("Fiber Temp");
	getStatistics(area, mean, min, max, std, histogram);
	//MinArea=1000;
	getVoxelSize(unit_per_pixel_width, unit_per_pixel_height, unit_per_pixel_depth, unit); // edited Aaron Yu 2021.12.08
	pixel_per_unit = Math.round(1/unit_per_pixel_width); // edited Aaron Yu 2021.12.08
	MinArea=0.05*area/pixel_per_unit; // edited Aaron Yu 2021.12.08
	...
	selectWindow("Fiber Temp");
	saveAs("tiff", ProcessedImagesDir + substring(newGlobalName, 0, lengthOf(newGlobalName) - 2) + "_artefact_detection");
	close();
...
function FiberShapeDetectionOnLaminin(FiberImageWindows,ROIFiberFile,ROICentroFiberFile,ROISatCellFiberFile,ROIVesselFiberFile,FiberArea,FiberFeret,MinFiberFeret,FiberAreaClassCount,nThreshold, bCrop) {
	...
	unit_conversion = 1;
	if (!setscale) {
		unit_conversion = convert_to_um(unit);
	}
	
	MinArea=150/unit_conversion; // edited Aaron Yu 2021.12.08
	if (bCrop) {
	...
	for (j=0; j<nbFibers; j++) {
		...
		AreaClassAttribution(FiberArea[j]*Math.sqr(unit_conversion),FiberAreaClassCount);
	...
	if (nbFibers > 0) {
		...
		print("nbSelectedFibers " + nbSelectedFibers);
		roiManager("Open", ROICentroFiberFile);
		AllFibersROI=roiManager("count");
		print("AllFibersROI " + AllFibersROI);
		selectWindow("Fiber Temp Segmented");
		selectWindow("ROI Manager");
		roiManager("Show All without labels");
...
//		print("vessel done");
	...
	selectWindow("Fiber Temp Segmented");
	saveAs("tiff", ProcessedImagesDir + substring(newGlobalName, 0, lengthOf(newGlobalName) - 2) + "_fiber_segmentation");
	run("Close");
	selectWindow("Fiber Temp");
	run("Close");
...
function CentroNucleiFiberTracking(ROIFiberFile,ROICentroFiberFile,ROIPeriFiberFile,DAPIWindows,CentroNuclei,PeriNuclei) {
	...
	selectWindow("DAPI Temp");
	saveAs("tiff", ProcessedImagesDir + substring(newGlobalName, 0, lengthOf(newGlobalName) - 2) + "_DAPI_nuclei");
...
//	run("Set Measurements...", "area mean centroid feret's redirect=None decimal=3");AllROI=roiManager("count")-1;
//		i=1;
//		while (i <= AllROI) {
//			roiManager("Select", 1);
//			roiManager("Delete");
...
//}
//
// new function and variable added - 2021.11.01 Aaron Yu
...
Dialog.setInsets(0, 0, 0);
Dialog.addMessage(" \nData Analysis\n=============");
//labels=newArray("Fiber Morphology", "Centro Nuclei Detection", "Fiber Type1", "Fiber Type2A","Satellite Cells","Fiber Type2B","Vessel","Intensity Only");
//defaults = newArray(1,1,0,0,0,0,0,0); // changed defaults 2021.11.01 by Aaron Yu
labels = newArray("Fiber Morphology", "Centro Nuclei Detection");
defaults = newArray(1, 1);
...
//items = newArray("None","Centro Nuclei Classes","Fiber Area Classes","Sat Cell","Fiber Types","Vessels");
items = newArray("None", "Centro Nuclei Classes");

Dialog.addChoice("Cartography Choice:", items,"Centro Nuclei Classes"); // changed defaults 2021.11.01 by Aaron Yu
...
Dots_per_micron_20X = 2*Dots_per_micron_10X;
Dots_per_micron_40X = Dots_per_micron_10X*4;

Dialog.addString("Dots per micron", Dots_per_micron_20X);
NameAnalysis=newArray("Fiber Morphology", "Centro Nuclei Detection", "Fiber Type1", "Fiber Type2A","Satellite Cells","Fiber Type2B","Vessel","Intensity Only");
Dialog.addChoice("Fiber Shape", colors, colors[2]); // changed to colors variable - 2021.11.01 Aaron Yu
for (i=1; i<= 7; i++) {
	if (checkAnalysis[i]) {
		if (i==4) {
			Dialog.addChoice("Nuclei Detection", colors); // changed to colors variable - 2021.11.01 Aaron Yu
		}
		if (i == 1) {
			Dialog.addChoice(NameAnalysis[i], colors, colors[3]); // default blue channel for nuclei detection - 2021.11.03 Aaron Yu
		} else {
			Dialog.addChoice(NameAnalysis[i], colors);// changed to colors variable - 2021.11.01 Aaron Yu
		}
	}
}
...
ProcessedImagesDir=arrivee+"/Processed_Images/";

if (!File.exists(ROIDir)) { // only create directories if they don't exist already - 2021.11.03 Aaron Yu
	File.makeDirectory(ArtefactDir);
	File.makeDirectory(ROIDir);
	File.makeDirectory(CartoDir);
	File.makeDirectory(ResultDir);

	File.makeDirectory(ProcessedImagesDir);
} else {
...
if (FileFormat=="TIFF (16bits) by channel") { ChannelFiber = parseInt(getChannelNumberFromColor(NumChannel[0]));} else { ChannelFiber = parseInt(getChannelNumberFromColor(NumChannel[0]))-1;} // changed to colors variable - 2021.11.01 Aaron Yu
...
if (checkAnalysis[1]) {
	if (FileFormat!="TIFF (16bits) by channel") ChannelNuclei = parseInt(getChannelNumberFromColor(NumChannel[1]))-1; else ChannelNuclei = parseInt(getChannelNumberFromColor(NumChannel[1])); // changed to colors variable - 2021.11.01 Aaron Yu
}
if (checkAnalysis[4]) {
	if (FileFormat!="TIFF (16bits) by channel") {
		ChannelNuclei = parseInt(getChannelNumberFromColor(NumChannel[1]))-1; // changed to colors variable - 2021.11.01 Aaron Yu
		ChannelSatCell= parseInt(getChannelNumberFromColor(NumChannel[4]))-1; // changed to colors variable - 2021.11.01 Aaron Yu
	}
	else {
		ChannelNuclei = parseInt(getChannelNumberFromColor(NumChannel[1])); // changed to colors variable - 2021.11.01 Aaron Yu
		ChannelSatCell= parseInt(getChannelNumberFromColor(NumChannel[4])); // changed to colors variable - 2021.11.01 Aaron Yu
	}
}

if (checkAnalysis[6]) {
	if (FileFormat!="TIFF (16bits) by channel") ChannelVessel= parseInt(getChannelNumberFromColor(NumChannel[6]))-1; else ChannelVessel= parseInt(getChannelNumberFromColor(NumChannel[6])); // changed to colors variable - 2021.11.01 Aaron Yu
}
if (checkAnalysis[2]) {
	if (FileFormat!="TIFF (16bits) by channel") ChannelType1= parseInt(getChannelNumberFromColor(NumChannel[2]))-1; else ChannelType1= parseInt(getChannelNumberFromColor(NumChannel[2])); // changed to colors variable - 2021.11.01 Aaron Yu
}
if (checkAnalysis[3]) {
	if (FileFormat!="TIFF (16bits) by channel") ChannelType2A= parseInt(getChannelNumberFromColor(NumChannel[3]))-1; else ChannelType2A= parseInt(getChannelNumberFromColor(NumChannel[3])); // changed to colors variable - 2021.11.01 Aaron Yu
}
if (checkAnalysis[5]) {
	if (FileFormat!="TIFF (16bits) by channel") ChannelType2B= parseInt(getChannelNumberFromColor(NumChannel[5]))-1; else ChannelType2B= parseInt(getChannelNumberFromColor(NumChannel[5])); // changed to colors variable - 2021.11.01 Aaron Yu
}
if (checkAnalysis[7]) {
	if (FileFormat!="TIFF (16bits) by channel") ChannelIntensity= parseInt(getChannelNumberFromColor(NumChannel[7]))-1; else ChannelIntensity= parseInt(getChannelNumberFromColor(NumChannel[7])); // changed to colors variable - 2021.11.01 Aaron Yu
}
...
for (ik=0; ik<list.length; ik=ik+NbChannels) {
	...
	FiberAreaClassCount = newArray(num_fiber_bins);
	...
	if (checkAnalysis[0]) {
		...
		if (FileFormat!="TIFF (16bits) by channel") {
			if (use_split_channels) {
				open(currentFile);
				CurrentWindows=getTitle();
				run("Split Channels");
				Title = CurrentWindows;
				newGlobalName = Title + " (";
				LamininWindows = newGlobalName + toLowerCase(NumChannel[0]) + ")";
			} else {
		...
		if (!bArtefacts) {
			...
			if (SurfaceTotFibers>0) {
				...
				roiManager("Show None");
				nbFibers = roiManager("count");
				FiberAreaMean = Math.sqr(unit_conversion)*SurfaceTotFibers/nbFibers;
				for (k=0 ; k<nbFibers; k++) {
					TotFiberFeret=TotFiberFeret+FiberFeret[k];
				}
	
				nFeretMean = unit_conversion*TotFiberFeret/nbFibers;
				if (ik==0) {
					output = "";
					for (i = 0; i < FiberAreaClassCount.length; i++) {
						if (i == 0){
							output = output + "\t<" + fiber_bin_class_min_area + "µm2";
						} else if (i == FiberAreaClassCount.length - 1) {
							output = output + "\t>" + fiber_bin_class_max_area + "µm2";
						} else {
							low = Math.round(fiber_bin_class_min_area + i*fiber_bin_class_separator);
							high = Math.round(fiber_bin_class_min_area + (i + 1)*fiber_bin_class_separator);
							output = output + "\t" + low + "-" + high + "µm2";
						}
					}
					TableResume[0]="FileName\tTotale Section Surface\tNb Segmented Fiber\tFiber Area Mean\tFiber Feret Mean" + output;
					NameGlobalResult =NameGlobalResult+"_FM";
					NameLamininResult = NameLamininResult+"_FM";
				}
				output = "";
				for (i = 0; i < FiberAreaClassCount.length; i++) {
					output = output + "\t" + FiberAreaClassCount[i];
				}
				TableResume[nCurrentLine]=GlobalName+"\t"+SurfaceTotSection+"\t"+nbFibers+"\t"+FiberAreaMean+"\t"+nFeretMean+output;
		...
		for (j=0; j < listdone.length; j++)
		{
			...
			if (filesearch == ROIFiberFile) {
				...
				if (FileFormat!="TIFF (16bits) by channel") {
					...
					Title = split(CurrentWindows,"=");
					newGlobalName=Title[0]+"=";
					LamininWindows=newGlobalName+(ChannelFiber - 1);
				}
				else {
					file1=depart+LamininWindows;
					run("Bio-Formats Importer", "open=file1 autoscale color_mode=Default view=Hyperstack stack_order=XYCZT");
				...
				for (k=0 ; k<nbFibers; k++) {
					...
					AreaClassAttribution(FiberArea[k]*Math.sqr(unit_conversion),FiberAreaClassCount);
				...
				if (ik==0) {
					...
					TableResume[0]="FileName\tNb Segmented Fiber\tFiber Area Mean\tFiber Feret Mean" + output;
				}
				output = "";
				for (i = 0; i < FiberAreaClassCount.length; i++) {
					output = output + "\t" + FiberAreaClassCount[i];
				}
				TableResume[nCurrentLine]=GlobalName+"\t"+nbFibers+"\t"+FiberAreaMean+"\t"+nFeretMean+output;
	...
	if(!filefound) {
		...
		if (checkAnalysis[1] || checkAnalysis[4])  {
			if (FileFormat !="TIFF (16bits) by channel") {
				if (use_split_channels) {
					DAPIWindows = newGlobalName + toLowerCase(NumChannel[1]) + ")";
				} else {
		...
		if (checkAnalysis[1]) {
			if (nbNuclei <nbFibers)  {
				...
				CNFAreaMean=0; // added 2021.11.01 - Aaron Yu
				NbCentroNucleiClass1=NbCentroNucleiClass2=NbCentroNucleiClass3=0; // moved to outside first if statement 2021.11.03 - Aaron Yu
				CN1Area=CN2Area=CN3Area=0; // moved to outside first if statement 2021.11.03 - Aaron Yu
				if (NbCentroNucleiCount>0) {
					for (i=0; i<nbFibers; i++) {
	 				...
	 				if (NbCentroNucleiClass1 >0) {
	 					CN1Area = CN1Area*Math.sqr(unit_conversion)/NbCentroNucleiClass1;
	 				}
	 				if (NbCentroNucleiClass2 >0) {
	 					CN2Area = CN2Area*Math.sqr(unit_conversion)/NbCentroNucleiClass2;
	 				}
	 				if (NbCentroNucleiClass3 >0) {
	 					CN3Area = CN3Area*Math.sqr(unit_conversion)/NbCentroNucleiClass3;
	 				}
	 				CNFAreaMean = CNFAreaMean*Math.sqr(unit_conversion)/NbCentroNucleiCount;
				}
		...
		// Fill table by fiber
		for (j=0; j<nbFibers; j++) {
			setResult("Area", j, FiberArea[j]*Math.sqr(unit_conversion));
			setResult("Max Feret", j, FiberFeret[j]*unit_conversion);
			setResult("Min Feret", j, MinFiberFeret[j]*unit_conversion);
			if (checkAnalysis[1]) {
			...
			if ((checkAnalysis[2]) || (checkAnalysis[3]) || (checkAnalysis[5]))  {
	...
	run("Close All");
	toc = getTime();
	print("Processing Time (s): " + ((toc - tic)/1000));
}
file=depart+list[0];
...
run("Close All");
close("ROI Manager");
print("Time Elapsed (s): " + ((toc - start_time)/1000));